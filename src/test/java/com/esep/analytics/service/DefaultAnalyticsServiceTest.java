...
class DefaultAnalyticsServiceTest {
    ...
    void shouldCalculatePercentagesFromAggregatedCategoryAmounts() {
        ...
        when(query.getCategoryExpenses()).thenReturn(List.of(
                new CategoryExpense("GROCERY", new BigDecimal("300.00"), BigDecimal.ZERO),
                new CategoryExpense("TRANSPORT", new BigDecimal("100.00"), BigDecimal.ZERO)
        ));
        ...
        assertThat(result).containsExactly(
                ...
                new CategoryExpense("TRANSPORT", new BigDecimal("100.00"), new BigDecimal("25.00"))
        );
    }
    ...
    void shouldReturnZeroPercentForEmptyExpenseTotal() {
        AnalyticsQuery query = mock(AnalyticsQuery.class);
        when(query.getCategoryExpenses()).thenReturn(List.of(
                new CategoryExpense("UNCATEGORIZED", BigDecimal.ZERO, BigDecimal.ONE)
        ));

        var result = new DefaultAnalyticsService(query).getCategoryExpenses();

        assertThat(result).containsExactly(new CategoryExpense("UNCATEGORIZED", BigDecimal.ZERO, BigDecimal.ZERO));
    }
}
